...
#				$item_old = array();
				...
				{
#					array_push($item_old, md5($row["title"]));
...
#					if(!in_array(md5($item["title"]), $item_old))
				...
				if($msg != "")
				{
					foreach($rss_receiver[$key] as $receiver)
					{
						...
						{
							$JABBER->SendMessage($receiver, "groupchat", NULL, array("body" => rtrim($msg)));
						}
						else
						{
							$JABBER->SendMessage($receiver, "chat", NULL, array("body" => rtrim($msg)));
						}
					}
				}
			}
